...
## Coordinates, scale, and source geometry
...
The ten overridden stations run from x = 0 to 108 in. The important plan change is that the 42 in sheer width occurs at x = 60 rather than x = 72. This lets the bow interface fall at the same station as the nominal 42 in center width. The transom remains 68 in wide in design space. The source cross-section sampling is increased to 30 bottom samples and 37 topside samples.
...
Every primary part is constructed directly as a closed solid. The code does not cut a four-piece assembly out of the older monolithic mesh because the Rev-2 monolith is not watertight.
...
The bow is therefore a hollow, manifold shell with an aft access opening, not a sealed pod. Near the joint, the cavity offset is deliberately increased so the bow-side flange ring is part of the cavity loft. Earlier code unioned a separate ring whose aft cap was coplanar with the shell face; float32 STL conversion collapsed the resulting zero-area triangles and the repair pass could bridge the thin shell and opening with unwanted membranes.
...
That is the correct model-scale number, and scaling the allowance up to preserve a 2 mm model gap would be a mistake. The allowance is sized for a tolerance stack the model does not have — laminate in the joint, bond lines across a chunked print, differential shrink over the 732 mm between the first and last wedge key — while the feature that makes a dovetail hold shrinks with everything else. `min_key_undercut()` returns the smallest undercut on the boat (7.5 mm real, 0.75 mm at 1:10, on the bow side keys). Clearance grows the socket on every face, so it consumes undercut one for one: at 2 mm on the model the capture margin is -1.25 mm and every key lifts straight out. `main()` prints the margin at both scales on every run and warns when the gap exceeds the undercut or falls below a printable fit.
...
The derived waterplane is a single horizontal z value. It replaces an older “waterline above local keel” rule which incorrectly followed approximately 13 in of keel rocker. `set_waterline_from_load()` runs before geometry construction because key, bolt, and sill rules depend on it.
...
Both use an ASA density of 1.07 g/cm3. This is a mass model of slicer behaviour, not a claim that every generated 7 or 14 mm wall is solid.
...
- **Piece topology:** `report()` checks `trimesh.is_watertight` for each finished piece. In this context the bow passes because a cup-like shell can be a closed manifold even though its storage mouth is open to its cavity; it is not being claimed as a sealed pod.
